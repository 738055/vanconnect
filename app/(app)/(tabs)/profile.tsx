...
export default function ProfileScreen() {
  ...
  const renderStarsRating = (rating: number) => {
    ...
    for (let i = 0; i < 5; i++) {
      if (i < fullStars) {
        ...
        stars.push(
          <Star key={i} size={16} color="#e5e7eb" />
        );
      }
    }
  ...
  const menuItems = [
    ...
    },
    {
      ...
      onPress: () => {/* Navigate to my transfers */},
    },
    {
        ...
        <View style={styles.header}>
          ...
          <View style={[styles.planBadge, { borderColor: getPlanColor() }]}>
            {subscription?.plan === 'enterprise' ? (
              <Crown size={18} color={getPlanColor()} />
            ) : subscription?.plan === 'pro' ? (
              <Star size={18} color={getPlanColor()} />
            ) : (
              <User size={18} color={getPlanColor()} />
            )}
